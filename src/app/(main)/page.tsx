  ...
  return (
    <div className="px-4 py-5">
      <header className="mb-5">
        ...
        <h1 className="mt-1 text-[24px] font-semibold leading-snug">
          {doneCount === 0
            ? '今天还没有开始'
      ...
      <section className="card mb-5 grid grid-cols-3 divide-x divide-line px-2 py-3.5">
        {[
          { value: summary.streak, unit: '天', label: '连续读经' },
          { value: summary.totalDevotions, unit: '次', label: '完整灵修' },
          { value: summary.totalNotes, unit: '条', label: '读经笔记' },
        ].map((s) => (
          <div key={s.label} className="px-2 text-center">
            <p className="text-[22px] font-semibold leading-none text-brand-500">
              {s.value}
      ...
      <section className="mb-5">
        <div className="mb-2 flex items-baseline justify-between">
          <h2 className="label">今日读经 · {settings.daily_chapters} 章</h2>
          <Link href="/me/settings" className="text-xs text-brand-500">
            调整
          </Link>
        </div>
      ...
      {ongoing.length > 0 && (
        <section className="mb-5">
          <h2 className="label mb-2">接着做</h2>
          <ul className="space-y-2">
            {ongoing.map((o) => (
              <li key={o.id}>
                <Link href={`/devotion/${o.id}`} className="card flex items-center justify-between px-4 py-3 active:bg-brand-50">
      ...
      <section className="card bg-brand-50/60 px-4 py-4">
        <p className="text-[13.5px] leading-[1.9] text-brand-700">
          翻完页不算读过。
          <br />
          先自己观察、自己提问、自己想过，
          <br />
        ...
        <Link
          ...
          className="btn-primary mt-3.5 w-full py-2.5"
        >
          开始今天的灵修
        </Link>
      </section>
