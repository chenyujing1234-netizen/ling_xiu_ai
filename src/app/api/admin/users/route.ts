...
export async function POST(req: Request) {
  return handler(async () => {
    ...
    const user = await conn
      ...
      .get<{ id: number; role: string; status: string }>(id);
    if (!user) bad('用户不存在');
    ...
    switch (action) {
      ...
      case 'disable':
        await conn.prepare(`UPDATE users SET status = 'disabled' WHERE id = ?`).run(id);
        return { ok: true };
      case 'enable':
        await conn.prepare(`UPDATE users SET status = 'active' WHERE id = ?`).run(id);
        return { ok: true };
      case 'promote':
        await conn.prepare(`UPDATE users SET role = 'admin' WHERE id = ?`).run(id);
        return { ok: true };
      case 'demote':
        await conn.prepare(`UPDATE users SET role = 'member' WHERE id = ?`).run(id);
        return { ok: true };
    }
  });
}
