...
一个**邀请制、无公开注册**的圣经读经灵修工具，面向教会小组 / 门训小组的小范围使用。移动端优先，底部四个 Tab（今日 / 读经 / 灵修 / 我的），可直接嵌入微信小程序的 `web-view`。
...
围绕同一章的辅助材料。原先是独立的"发现" Tab，但它整理的东西和灵修都以"章"为单位，分成两处只会让人来回跳，所以并进灵修页。
...
**鉴权与隐私**
- 未登录访问 `/`、`/read`、`/devotion`、`/me`、`/admin` 全部 307 跳登录；API 返回 401
