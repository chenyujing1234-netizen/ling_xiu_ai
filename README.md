...
**其它**：选段配图 23 项自测全通（真生成 23.4s、二次命中 66ms、只圈一节也能出图、选段时长按不抢手势、退出后长按照旧）；读经并入灵修页后 31 项浏览器自测全通（默认停读经、三页签互切、两层吸顶不重叠、切走再切回仍停在原来那一章、`/read` 与 `/explore` 老书签自动转新地址）；中英对照逐节正确；上下文窗口正确跨章（约 3:16 → 前到 3:6、后到 3:26）；知识图谱 15 节点 22 边无孤立节点无非法边；思维导图 3 层 20 叶子；洞察缓存二次命中 0.01s；移动端底部 Tab、安全区、`viewport-fit=cover` 均到位。
